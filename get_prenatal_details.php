...
try {
    ...
    <!-- ALL VISITS - Laboratory Results (Consolidated from all visits) -->
    <div class="detail-section mt-4">
        <h6><i class="fas fa-flask me-2"></i>Laboratory Results - All Visits</h6>
        ...
        <div class="table-responsive">
            <table class="table table-sm table-bordered">
                ...
                <tbody>
                    ...
                    <?php 
                    // Check if this visit has any lab results
                    $hasLabResults = !empty($visit['hb_level']) || !empty($visit['blood_group']) || 
                                   !empty($visit['rhesus_factor']) || !empty($visit['blood_sugar']) ||
                                   !empty($visit['hiv_status']) || !empty($visit['hepatitis_b']) ||
                                   !empty($visit['vdrl']) || !empty($visit['urinalysis']) ||
                                   !empty($visit['other_tests']);
                    ?>
                    <?php if ($hasLabResults): ?>
                    <tr>
                        <td>
                            <strong>Visit <?= $visit['visit_number'] ?? 'N/A' ?></strong>
                            <?php if ($visit['id'] == $recordId): ?>
                            <span class="badge bg-primary">Current</span>
                            <?php endif; ?>
                        </td>
                        <td><?= !empty($visit['visit_date']) ? date('M j, Y', strtotime($visit['visit_date'])) : 'N/A' ?></td>
                        <td><?= !empty($visit['hb_level']) ? $visit['hb_level'] . ' g/dL' : 'N/A' ?></td>
                        <td><?= !empty($visit['blood_group']) ? htmlspecialchars($visit['blood_group']) : 'N/A' ?></td>
                        <td><?= !empty($visit['rhesus_factor']) ? htmlspecialchars($visit['rhesus_factor']) : 'N/A' ?></td>
                        <td><?= !empty($visit['blood_sugar']) ? $visit['blood_sugar'] . ' mg/dL' : 'N/A' ?></td>
                        <td><?= !empty($visit['hiv_status']) ? htmlspecialchars($visit['hiv_status']) : 'N/A' ?></td>
                        <td><?= !empty($visit['hepatitis_b']) ? htmlspecialchars($visit['hepatitis_b']) : 'N/A' ?></td>
                        <td><?= !empty($visit['vdrl']) ? htmlspecialchars($visit['vdrl']) : 'N/A' ?></td>
                        <td><?= !empty($visit['urinalysis']) ? htmlspecialchars(substr($visit['urinalysis'], 0, 50)) . (strlen($visit['urinalysis']) > 50 ? '...' : '') : 'N/A' ?></td>
                        <td><?= !empty($visit['other_tests']) ? htmlspecialchars(substr($visit['other_tests'], 0, 50)) . (strlen($visit['other_tests']) > 50 ? '...' : '') : 'N/A' ?></td>
                    </tr>
                    <?php endif; ?>
                    <?php endforeach; ?>
                    ...
                    <tr>
                        <td colspan="11" class="text-center">
                            <span class="empty-data">No laboratory results recorded for any visit</span>
                        </td>
                    </tr>
                    <?php endif; ?>
                </tbody>
    ...
    <!-- ALL VISITS - Treatments Given (Consolidated from all visits) -->
    <div class="detail-section mt-4">
        <h6><i class="fas fa-pills me-2"></i>Treatments Given - All Visits</h6>
        ...
        <div class="table-responsive">
            <table class="table table-sm table-bordered">
                ...
                <tbody>
                    <?php foreach ($allVisits as $visit): ?>
                    <?php 
                    // Check if this visit has any treatment information
                    $hasTreatment = !empty($visit['diagnosis']) || !empty($visit['treatment']) || 
                                  !empty($visit['iron_supplement']) || !empty($visit['folic_acid']) ||
                                  !empty($visit['calcium']) || !empty($visit['other_meds']);
                    ?>
                    <?php if ($hasTreatment): ?>
                    <tr>
                        <td>
                        ...
                        <td><?= !empty($visit['visit_date']) ? date('M j, Y', strtotime($visit['visit_date'])) : 'N/A' ?></td>
                        <td><?= !empty($visit['diagnosis']) ? nl2br(htmlspecialchars($visit['diagnosis'])) : 'N/A' ?></td>
                        <td><?= !empty($visit['treatment']) ? nl2br(htmlspecialchars($visit['treatment'])) : 'N/A' ?></td>
                        <td>
                            <?php 
                            $meds = [];
                            if (!empty($visit['iron_supplement']) && $visit['iron_supplement']) $meds[] = 'Iron';
                            if (!empty($visit['folic_acid']) && $visit['folic_acid']) $meds[] = 'Folic Acid';
                            if (!empty($visit['calcium']) && $visit['calcium']) $meds[] = 'Calcium';
                            if (!empty($visit['other_meds'])) $meds[] = 'Other: ' . substr($visit['other_meds'], 0, 30) . (strlen($visit['other_meds']) > 30 ? '...' : '');
                            
                            echo !empty($meds) ? implode(', ', $meds) : 'N/A';
                            ?>
                        </td>
                        <td>
                            <?= !empty($visit['recorded_first_name']) ? 
                                htmlspecialchars(($visit['recorded_first_name'] ?? '') . ' ' . ($visit['recorded_last_name'] ?? '')) : 
                                'System' ?>
                        </td>
                    </tr>
                    <?php endif; ?>
                    ...
                    <tr>
                        <td colspan="6" class="text-center">
                            <span class="empty-data">No treatments recorded for any visit</span>
                        </td>
                    </tr>
                    <?php endif; ?>
                </tbody>
